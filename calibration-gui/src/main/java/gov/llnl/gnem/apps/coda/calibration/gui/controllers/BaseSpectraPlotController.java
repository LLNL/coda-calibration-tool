...
public class BaseSpectraPlotController<T> {
    private final SpectralPlot spectraPlot = new SpectralPlot();
    private boolean isYaxisResizble = false;
    private final Function<T, Double> dataFunction;
    private boolean shouldShowFits = false;
    private final Map<Point2D, T> spectraDataMap = new HashMap<>();

    public BaseSpectraPlotController(final Function<T, Double> dataFunction) {
    ...
    public Map<Point2D, T> getSpectraDataMap() {
    ...
    public Function<T, Double> getDataFunc() {
        return dataFunction;
    }
