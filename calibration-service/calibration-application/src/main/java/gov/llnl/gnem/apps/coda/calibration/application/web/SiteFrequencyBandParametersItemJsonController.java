...
public class SiteFrequencyBandParametersItemJsonController {

    @Autowired
    public SiteFrequencyBandParametersItemJsonController(SiteFrequencyBandParametersService siteFrequencyBandParametersService) {
    ...
    public ResponseEntity<?> show(@ModelAttribute SiteFrequencyBandParameters siteFrequencyBandParameters) {
        return ResponseEntity.ok(siteFrequencyBandParameters);
    }

    public static UriComponents showURI(SiteFrequencyBandParameters siteFrequencyBandParameters) {
        return MvcUriComponentsBuilder.fromMethodCall(MvcUriComponentsBuilder.on(SiteFrequencyBandParametersItemJsonController.class).show(siteFrequencyBandParameters))
                                      .buildAndExpand(siteFrequencyBandParameters.getId())
                                      .encode();
    }

    @PutMapping(name = "update")
    public ResponseEntity<?> update(@ModelAttribute SiteFrequencyBandParameters storedSiteFrequencyBandParameters, @Valid @RequestBody SiteFrequencyBandParameters siteFrequencyBandParameters,
            BindingResult result) {

        if (result.hasErrors()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(result);
        }
        siteFrequencyBandParameters.setId(storedSiteFrequencyBandParameters.getId());
        getSiteFrequencyBandParametersService().save(siteFrequencyBandParameters);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping(name = "delete")
    public ResponseEntity<?> delete(@ModelAttribute SiteFrequencyBandParameters siteFrequencyBandParameters) {
        getSiteFrequencyBandParametersService().delete(siteFrequencyBandParameters);
        return ResponseEntity.ok().build();
    }
