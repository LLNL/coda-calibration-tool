...
@RestController
@CrossOrigin
@RequestMapping(value = { "/api/v1/validation-events", "/api/v1/validation-events/" }, name = "ValidationEventsJsonController", produces = MediaType.APPLICATION_JSON_VALUE)
public class ValidationEventsJsonController {

    private ValidationMwParametersService service;

    @Autowired
    public ValidationEventsJsonController(ValidationMwParametersService service) {
        this.service = service;
    }

    @GetMapping(name = "getValidationEvents")
    public List<ValidationMwParameters> getValidationEvents() {
        return service.findAll();
    }

    @PostMapping(value = { "/batch", "/batch/" }, name = "createBatch")
    public ResponseEntity<?> createBatch(@Valid @RequestBody Collection<ValidationMwParameters> referenceEvents, BindingResult result) {
        if (result.hasErrors()) {
    ...
    public ValidationMwParametersService getService() {
        return service;
    }

    public void setService(ValidationMwParametersService service) {
        this.service = service;
    }
