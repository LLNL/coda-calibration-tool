...
@RestController
@CrossOrigin
@RequestMapping(value = { "/api/v1/shape-measurements", "/api/v1/shape-measurements/" }, name = "ShapeMeasurementJsonController", produces = MediaType.APPLICATION_JSON_VALUE)
public class ShapeMeasurementJsonController {
    ...
    public List<ShapeMeasurement> getMeasurementsMetadata() {
        return service.findAllMetadataOnly().stream().map(ShapeMeasurement::new).collect(Collectors.toList());
    }
