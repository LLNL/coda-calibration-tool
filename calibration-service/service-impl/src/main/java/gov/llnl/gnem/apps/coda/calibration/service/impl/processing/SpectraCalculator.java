...
public class SpectraCalculator {
	...
	public SpectraCalculator(WaveformToTimeSeriesConverter converter, SyntheticCodaModel syntheticCodaModel,
			EndTimePicker endTimePicker, MdacCalculatorService mdacService, MdacParametersFiService mdacFiService,
	...
	public List<SpectraMeasurement> measureAmplitudes(List<SyntheticCoda> generatedSynthetics,
			Map<FrequencyBand, SharedFrequencyBandParameters> frequencyBandParameterMap, Boolean autoPickingEnabled) {
	...
	public List<SpectraMeasurement> measureAmplitudes(List<SyntheticCoda> generatedSynthetics,
		...
		return generatedSynthetics	.parallelStream()
									.map(synth -> measureAmplitudeForSynthetic(	synth, frequencyBandParameterMap,
																				frequencyBandSiteParameterMap,
																				autoPickingEnabled))
									.filter(Objects::nonNull).collect(Collectors.toList());
	}

	private SpectraMeasurement measureAmplitudeForSynthetic(SyntheticCoda synth,
			Map<FrequencyBand, SharedFrequencyBandParameters> frequencyBandParameterMap,
			Map<FrequencyBand, Map<Station, SiteFrequencyBandParameters>> frequencyBandSiteParameterMap,
			Boolean autoPickingEnabled) {

		FrequencyBand frequencyBand = new FrequencyBand(synth.getSourceWaveform().getLowFrequency(),
		...
		if (params != null) {
			...
			TimeSeries synthSeis = new TimeSeries(	WaveformUtils.doublesToFloats(synth.getSegment()),
													synth.getSampleRate(), new TimeT(synth.getBeginTime()));
			...
			double distance = EModel.getDistanceWGS84(	event.getLatitude(), event.getLongitude(), station.getLatitude(),
			...
			if (frequencyBandSiteParameterMap != null && frequencyBandSiteParameterMap.containsKey(frequencyBand)
					&& frequencyBandSiteParameterMap.get(frequencyBand).get(station) != null) {
			...
			double eshCorrection = log10ESHcorrection(	synth.getSourceWaveform().getLowFrequency(),
														synth.getSourceWaveform().getHighFrequency(), params.getS1(),
														params.getS2(), params.getXc(), params.getXt(), params.getQ(),
														distance);
			double noiseamp;
			try {
				// measure the amplitude in the noise window the end cut is
				// defined where the data envelope falls below minSNR (usually
				// 2*noise-amp (0.3 in log10 space))
				noiseamp = WaveformUtils.getNoiseWindow(distance, originTime, envSeis).getMean();
			} catch (IllegalArgumentException e) {
				log.warn(	"Error attempting to measure seismogram amplitude for seismogram {}; {}", synth,
							e.getMessage());
				return null;
			}
			...
			if (synth.getSourceWaveform().getAssociatedPicks() != null) {
				endTime = synth	.getSourceWaveform().getAssociatedPicks().stream()
								.filter(p -> PICK_TYPES.F.name().equalsIgnoreCase(p.getPickType())).findFirst()
								.map(pick -> {
									double startpick = startTime.subtractD(originTime);
									TimeT result = originTime;
									result = pick.getPickTimeSecFromOrigin() > (startpick + maxlength)
											? result.add(startpick + maxlength)
											: result.add(pick.getPickTimeSecFromOrigin());
									return result;
								}).orElse(null);
			}

			if (endTime == null && autoPickingEnabled) {
				endTime = new TimeT(endTimePicker.getEndTime(	envSeis.getData(), synthSeis.getData(),
																envSeis.getSamprate(), startTime.getEpochTime(),
																envSeis.getIndexForTime(startTime.getEpochTime()),
																minlength, maxlength, params.getMinSnr(), noiseamp,
																params.getLowFrequency() + ((params.getHighFrequency()
																		- params.getLowFrequency()) / 2.0),
																distance));
			}
			...
			if (startTime.ge(endTime)) {
				log.trace(	"Coda envelope start time is after the seismograms end time, start: {} end: {} for envelope: {}",
							startTime.getDate(), endTime.getDate(), synth.getSourceWaveform());
				return null;
			} else if (endTime.le(startTime)) {
				log.trace(	"Coda envelope end time is before the start of the seismogram, start: {} end: {} for envelope: {}",
							startTime.getDate(), endTime.getDate(), synth.getSourceWaveform());
				return null;
			}
			...
			if (cutSucceeded) {
				...
				double rawAmp = new TimeSeries(	SeriesMath.Subtract(envdata, synthdata), synthSeis.getSamprate(),
												synthSeis.getTime()).getMedian();
				...
				if (measurementtime > 0.) {
					double shiftedvalue = syntheticCodaModel.getPointAtTimeAndDistance(	params, measurementtime,
																						distance);
				...
				return new SpectraMeasurement()	.setWaveform(synth.getSourceWaveform()).setRawAtStart(rawAmp)
												...
												.setEndCutSec(endTime.getEpochTime()).setRmsFit(fit);
			}
		}
	 ...
	 */
	public double log10ESHcorrection(double lowfreq, double highfreq, double alpha1, double alpha2, double xc,
			double xt, double q, double distance) {
	...
	public double log10ESHcorrection(double s1, double s2, double xcross, double xtrans, double distance) {
		...
		if (distance <= xstart) {
			...
			double ds = s2 - s1;
			eshmod = -1.0 * s1 * Math.log10(xstart) - (s1 + ds / 2.) * Math.log10(xend / xstart)
					- s2 * Math.log10(distance / xend);
		} else {
	...
	public Spectra computeReferenceSpectra(ReferenceMwParameters refEvent, List<FrequencyBand> bands,
		...
		for (FrequencyBand band : bands) {
			...
			if (refEvent.getStressDropInMpa() != null && refEvent.getStressDropInMpa() > 0.0) {
				...
				amplitude = mdacService.calculateMdacAmplitudeForMw(psRows, mdacFiEntry, refEvent.getRefMw(),
																	centerFreq, selectedPhase,
																	refEvent.getStressDropInMpa());
			} else {
				amplitude = mdacService.calculateMdacAmplitudeForMw(psRows, mdacFiEntry, refEvent.getRefMw(),
																	centerFreq, selectedPhase);
			}
	...
	public Spectra computeFitSpectra(MeasuredMwParameters event, List<FrequencyBand> bands, PICK_TYPES selectedPhase) {
		...
		for (FrequencyBand band : bands) {
			...
			if (event.getStressDropInMpa() != null && event.getStressDropInMpa() > 0.0) {
				...
				mdacFiEntry.setPsi(0.0);
				amplitude = mdacService.calculateMdacAmplitudeForMw(psRows, mdacFiEntry, event.getMw(), centerFreq,
																	selectedPhase, event.getStressDropInMpa());
			} else {
				amplitude = mdacService.calculateMdacAmplitudeForMw(psRows, mdacFiEntry, event.getMw(), centerFreq,
																	selectedPhase);
			}
	...
	public List<MeasuredMwParameters> measureMws(final Map<Event, Map<FrequencyBand, SummaryStatistics>> evidMap,
		...
		for (Entry<Event, Map<FrequencyBand, SummaryStatistics>> entry : evidMap.entrySet()) {
			...
			measuredMws.add(new MeasuredMwParameters()	.setEventId(entry.getKey().getEventId()).setMw(MoMw[1])
														.setStressDropInMpa(MoMw[4]));
		}
		return measuredMws;
	}
	 ...
	 */
	public double[] fitMw(final Map<FrequencyBand, SummaryStatistics> measurements, final PICK_TYPES phase,
			final MdacParametersFI mdacFi, final MdacParametersPS mdacPs) {
			...
			public double value(double[] point) {
				...
				for (Entry<FrequencyBand, SummaryStatistics> meas : measurements.entrySet()) {
					...
					if (mdacPs != null) {
						...
						double log10mdacM0 = mdacService.calculateMdacAmplitudeForMw(	mdacPs, mdacFi, testMw, centerFreq,
																						phase, testSigma);
		...
		CMAESOptimizer optimizer = new CMAESOptimizer(	1000000, 0, true, 0, 10, new MersenneTwister(), true,
														convergenceChecker);
		PointValuePair optimizerResult = optimizer.optimize(new MaxEval(1000000), new ObjectiveFunction(mdacFunction),
															GoalType.MINIMIZE,
															new SimpleBounds(	new double[] { minMW, minMPA },
																				new double[] { maxMW, maxMPA }),
															new InitialGuess(new double[] {
																	ThreadLocalRandom.current().nextDouble(	minMW,
																											maxMW),
																	ThreadLocalRandom.current().nextDouble(	minMPA,
																											maxMPA) }),
