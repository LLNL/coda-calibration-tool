...
public class SpectraCalculator {
    ...
    private SpectraMeasurement measureAmplitudeForSynthetic(SyntheticCoda synth, Map<FrequencyBand, SharedFrequencyBandParameters> frequencyBandParameterMap,
        ...
        if (params != null) {
            ...
            try {
                ...
                log.warn("Error attempting to measure seismogram amplitude for seismogram {}; {}", synth, e.getMessage());
                return null;
            }
            ...
            if (cutSucceeded) {
                ...
                return new SpectraMeasurement().setWaveform(synth.getSourceWaveform())
                                               ...
                                               .setEndCutSec(startTime.subtract(originTime).getEpochTime() + endTime.subtract(startTime).getEpochTime())
                                               .setRmsFit(fit);
            }
        }
    ...
    private boolean cutSeismograms(TimeSeries a, TimeSeries b, TimeT startTime, TimeT endTime) {
        ...
        try {
            ...
            // These might be off by some small number of samples due to precision
            // errors during cut and SeriesMath will throw an ArrayBounds if they
            ...
            if (a.getNsamp() != b.getNsamp()) {
                ...
                // choose the latest start time and the earliest end time for the
                // cut window
                if (startA.lt(startB)) {
                    start = startB;
                }
    ...
    public Spectra computeReferenceSpectra(ReferenceMwParameters refEvent, List<FrequencyBand> bands, PICK_TYPES selectedPhase) {
        ...
        for (FrequencyBand band : bands) {
            ...
            if (refEvent.getStressDropInMpa() != null && refEvent.getStressDropInMpa() > 0.0) {
                //If we know a MPA stress drop for this reference event we want to use stress instead of apparent stress
