...
public class ShapeCalibrationServiceImpl implements ShapeCalibrationService {
	...
	public ShapeCalibrationServiceImpl(ShapeMeasurementService shapeMeasurementService, ShapeCalculator shapeCalc,
			EndTimePicker endTimePicker, SyntheticCodaModel syntheticCodaModel, WaveformService waveService,
	...
	@Override
	public Map<FrequencyBand, SharedFrequencyBandParameters> measureShapes(
			Collection<PeakVelocityMeasurement> velocityMeasurements,
		...
		List<ShapeMeasurement> betaAndGammaMeasurements = shapeCalc.fitShapelineToMeasuredEnvelopes(filteredVelocityMeasurements,
																									frequencyBandCurveFits);
	...
	private Collection<PeakVelocityMeasurement> autoPickWaveforms(
			final Collection<PeakVelocityMeasurement> velocityMeasurements,
			final Map<FrequencyBand, SharedFrequencyBandParameters> frequencyBandParameters) {
		return velocityMeasurements	.parallelStream().filter(vel -> vel.getWaveform() != null)
									.filter(vel -> vel.getWaveform().getAssociatedPicks() != null).map(vel -> {
										SharedFrequencyBandParameters params = frequencyBandParameters.get(new FrequencyBand(	vel.getWaveform()
																																.getLowFrequency(),
																																vel	.getWaveform()
																																	.getHighFrequency()));
										Optional<WaveformPick> pick = vel	.getWaveform().getAssociatedPicks().stream()
																			.filter(p -> PICK_TYPES.AP	.name()
																										.equalsIgnoreCase(p.getPickType()))
																			.findFirst();

										Optional<WaveformPick> endPick = vel.getWaveform().getAssociatedPicks().stream()
										...
										if ((!endPick.isPresent() || pick.isPresent()) && params != null) {
											...
											double noiseamp;
											try {
												noiseamp = WaveformUtils.getNoiseWindow(vel.getDistance(),
																						new TimeT(vel	.getWaveform()
																										.getEvent().getOriginTime()),
																						converter.convert(vel.getWaveform()))
																		.getMean();
											} catch (IllegalArgumentException e) {
												log.warn(	"Error attempting to measure seismogram amplitude for seismogram {}; {}",
															vel.getWaveform(), e.getMessage());
												return null;
											}
											...
											double vr = params.getVelocity0() - params.getVelocity1()
													/ (params.getVelocity2() + vel.getDistance());
											if (vr == 0.0) {
												vr = 1.0;
											}
											...
											double stopTime = endTimePicker.getEndTime(	segment.getData(),
																						WaveformUtils.doublesToFloats(quickSynth(	vel.getWaveform(),
																																	startTime).getSegment()),
																						segment.getSamprate(),
																						startTime.getEpochTime(),
																						segment.getIndexForTime(startTime.getEpochTime()),
																						minlength, maxlength,
																						params.getMinSnr(), noiseamp,
																						params.getLowFrequency()
																								+ ((params.getHighFrequency()
																										- params.getLowFrequency())
																										/ 2.0),
																						vel.getDistance());
	 ...
	 */
	private Collection<Entry<PeakVelocityMeasurement, WaveformPick>> filterMeasurementsToEndPickedOnly(
			Collection<PeakVelocityMeasurement> velocityMeasurements) {
		return velocityMeasurements	.parallelStream().filter(vel -> vel.getWaveform() != null)
									.filter(vel -> vel.getWaveform().getAssociatedPicks() != null).map(vel -> {
