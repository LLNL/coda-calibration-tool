 ...
 * pick. VPickLines can be moved interactively, and can be queried for their value.
 * VPickLine also has associated error bars and an associated window. The error
 * bars and the window can be turned on and off as required.
 *
 * @author Doug Dodge
 */
public class VPickLine extends PlotObject {
    /**
     * Gets the data value (real-world value) currently represented by this VPickLine
     * object.
     *
    ...
    /**
     * Gets the error value associated with the VPickLine object. This is the absolute
     * value of the difference between the VPickLine data value and the data value
     * of one of the associated error bars.
     *
    ...
    /**
     * Gets the draggable attribute of the VPickLine object. This attribute controls
    ...
    /**
     * Gets the windowHandle attribute of the VPickLine object. This is the object
     * that allows interaction with the right side of the window associated with this
     * pick.
     *
    ...
    /**
     * Gets a reference to the associated JWindowRegion. This can be used to set color
     * values etc.
     *
    ...
    /**
     * Gets the dataBridge associated with the VPickLine object. If no PickDataBridge
    ...
    /**
     * Sets the X-position of the VPickLine in real-world coordinates. Does not cause
     * a re-render of the object.
     *
    ...
    /**
     * Sets the y-position of the center of the VPickLine. This method only has an
     * effect if the VPickLine was instantiated using a constructor that specifies
     * both x and y values. Changes made using this method do not cause re-rendering
     * of the object.
     *
    ...
    /**
     * Sets the height of the VPickLine object in real-world coordinates. This method
     * only has an effect if the VPickLine was instantiated using a constructor that
     * specifies both x and y values and pick height. Changes made using this method
     * do not cause re-rendering of the object.
     *
    ...
    /**
     * Controls whether the VPickLine can be adjusted by dragging with the mouse.
     *
    ...
    /**
     * Sets the text Position of the VPickLine object. Text can be displayed either
     * above the pick line or below the line.
     *
    ...
    /**
     * Sets the value of the error for the associated VPickLineErrorBar objects. This
     * method does not cause the error bars to be re-rendered.
     *
    ...
    /**
     * Sets the bracket Width of the associated VPickLineErrorBar objects. Each error
     * bar has an inward-facing bracket at top and bottom. The width of this bracket
    ...
    /**
     * Sets the handle Width of the associated VPickLineErrorBar objects. Each error
     * bar has an optional square handle that may make it easier to select with the
    ...
    /**
     * Controls whether the error-bar handles are displayed. The error bars have optional
     * box-like handles that make it easier to grab them, particularly when the std
    ...
    /**
     * Sets the showErrorBars attribute of the VPickLine object. When this attribute
     * is true, a pair of adjustable error bars will be displayed along with the pick
     * line.
     *
    ...
    /**
     * Constructor for the VPickLine object that specifies a fixed height and specific
     * x and y values for the VPickLine object and which leaves other values to defaults
     *
    ...
    /**
     * Constructor for the VPickLine object that specifies a fixed height and specific
     * x and y values for the VPickLine object and which allows other parameters to
     * be set.
     *
     * @param xval      The x-value at which to place the VPickLine object
     * @param yval      The y-value of the center of the VPickLine object
    ...
    /**
     * Constructor for the VPickLine object that creates a VPickLine which adaptively
     * sizes itself to the current plot dimensions and which leaves other parameters
     * at default values.
     *
     * @param xval          The x-value at which to place the VPickLine object
     * @param yAxisFraction The fraction of the JSubplot height that should be occupied
     *                      by the VPickLine object
     * @param text          The text to associate with the VPickLine object.
     */
    ...
    /**
     * Constructor for the VPickLine object that creates a VPickLine that adaptively
     * sizes itself to the current plot dimensions and that allows other parameters
     * to be set.
     *
     * @param xval          The x-value at which to place the VPickLine object
     * @param yAxisFraction The fraction of the JSubplot height that should be occupied
    ...
    /**
     * Constructor for the VPickLine object that installs a PickDataBridge observer
     * into the VPickLine and that adaptively sizes itself to the current plot dimensions
     * and that allows other parameters to be set.
     *
    ...
    /**
     * Constructor for the VPickLine object that installs a PickDataBridge observer
     * into the VPickLine and that creates a pick with a specific y-value and height in mm.
     *
     * @param dataBridge    Description of the Parameter
     * @param text          The text to associate with the VPickLine object.
     * @param color         The color of the VPickLine object
    ...
    /**
     * Sets the forceInBounds attribute of the VPickLine object. When this attribute
     * is true, the VPickLine can only have values greater than lowerBound. This capability
     * supports applications that need to control the adjustment of picks.
     *
    ...
    /**
     * Sets the lowerBound attribute of the VPickLine object. When forceInBounds is
     * true, the VPickLine can only have values greater than lowerBound. This capability
     * supports applications that need to control the adjustment of picks.
     *
    ...
    /**
     * Sets the selected state of the VPickLine object. This causes a re-rendering
    ...
    /**
     * render this VPickLine object and its contained PlotObjects to the supplied graphics
     * context.
     *
    ...
    /**
     * This is an over-ride of the PlotObject::hasContainedObjects method. During
     * rendering, the object is queried to determine whether it contains any sub-objects
     * that must be rendered. If so, the PlotObject is asked to produce a vector of
     * its contained PlotObjects.
     *
     * @return This method always returns true for VPickLine since VPickLine contains
     *         pick error bars, a window and a window handle.
     */
    @Override
    ...
    /**
     * Gets a vector of contained PlotObjects. This is used during rendering of the
     * VPickLine, and allows its contained PlotObjects to be rendered.
     *
    ...
    /**
     * Gets the pixel value of the bottom of the pick line. This is used by associated
     * objects during their rendering. This value is reset each time the VPickLine
     * is rendered.
     *
    ...
    /**
     * Gets the pixel value of the top of the pick line. This is used by associated
     * objects during their rendering. This value is reset each time the VPickLine
     * is rendered.
     *
    ...
    /**
     * Updates the position of a VPickLineErrorBar when the other one has been changed.
     * This allows the pair to move in sync.
     *
     * @param changedBar The VPickLineErrorBar which has had its position changed.
    ...
    /**
     * Given a reference to a VPickLineErrorBar object, returns a reference to the
     * other VPickLineErrorBar associated with this pick. The input reference must
     * be for one of the two VPickLineErrorBar objects associated with this pick.
     *
