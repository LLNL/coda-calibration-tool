...
public class EnvelopeWebClient implements EnvelopeClient {
    ...
    public Flux<Waveform> getAllEnvelopes() {
        return client.get().uri("/waveforms/query/all").accept(MediaType.APPLICATION_JSON).exchange().flatMapMany(response -> response.bodyToFlux(Waveform.class)).onErrorReturn(new Waveform());
    }
}
