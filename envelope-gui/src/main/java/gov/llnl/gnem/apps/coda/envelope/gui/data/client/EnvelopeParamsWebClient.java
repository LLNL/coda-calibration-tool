...
public class EnvelopeParamsWebClient implements EnvelopeParamsClient {
    ...
    public Mono<EnvelopeJobConfiguration> getEnvelopeJobConfiguration() {
        return client.get().uri("/envelopes/job-configuration").accept(MediaType.APPLICATION_JSON).exchange().doOnSuccess(resp -> {
            if (!HttpStatus.OK.equals(resp.statusCode())) {
                throw new LightweightIllegalStateException(resp.toString());
            }
        }).flatMap(response -> response.bodyToMono(EnvelopeJobConfiguration.class));
    }
}
